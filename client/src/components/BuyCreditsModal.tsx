  ...
  return (
    ...
    >
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        {/* Header */}
        <div className="bg-gradient-to-r from-[#3a485b] to-[#0f766e] px-6 py-5 text-white">
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 gap-2 mb-5">
            {PACKS.map((p) => (
              ...
              >
                <div>
                  <p className={`text-sm font-bold ${selected === p.id ? "text-[#0f766e]" : "text-[#3a485b]"}`}>
                    {p.credits} Credits
                  </p>
                  <p className="text-sm text-gray-700">{p.note} · {p.perCredit}</p>
                </div>
                <p className={`text-lg font-bold ${selected === p.id ? "text-[#0f766e]" : "text-gray-700"}`}>
                  ${p.price.toFixed(2)}
                </p>
                {p.id === "50" && (
                  <span className="absolute -top-2 -right-2 bg-[#0f766e] text-white text-xs font-bold px-1.5 py-0.5 rounded-full">
                    Popular
                  </span>
                )}
              </button>
          ...
          <div className="bg-teal-50 border border-teal-100 rounded-xl px-4 py-3 mb-4 flex items-center justify-between">
            ...
            <span className="text-lg font-bold text-[#0f766e]">${pack.price.toFixed(2)}</span>
          </div>
          ...
          <button
            onClick={handleBuy}
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-bold bg-[#0f766e] text-white hover:bg-[#115e59] transition disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
