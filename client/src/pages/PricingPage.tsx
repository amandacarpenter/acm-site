...
export default function PricingPage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Individual plans */}
      <section className="py-16 sm:py-20 bg-[#3a485b]" aria-labelledby="individual-heading">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <h2 id="individual-heading" className="text-center text-xs font-bold uppercase tracking-widest text-white/50 mb-10">
            Individual Plans
          </h2>
            ...
            {INDIVIDUAL_PLANS.map((plan) => (
              <div
                ...
                className={`rounded-2xl p-7 relative flex flex-col ${
                  plan.highlight
                    ? "bg-[#0d9488] shadow-2xl scale-[1.03]"
              ...
              >
                ...
                <div className="mb-5">
                  <p className={`text-xs font-bold uppercase tracking-widest mb-3 ${plan.highlight ? "text-white/70" : "text-white/50"}`}>
                    {plan.name}
                  </p>
                  <div className="flex items-end gap-1 mb-1">
                    ...
                    <span className={`mb-1 text-sm ${plan.highlight ? "text-white/70" : "text-white/50"}`}>/mo</span>
                  </div>
                  {annual ? (
                    <p className={`text-xs ${plan.highlight ? "text-white/60" : "text-white/40"}`}>
                ...
                <p className={`text-xs leading-relaxed mb-4 ${plan.highlight ? "text-white/70" : "text-white/50"}`}>
                  {plan.description}
                </p>

                {/* Doc limit badge */}
                <div className={`inline-flex items-center gap-1.5 text-xs font-semibold rounded-full px-3 py-1 mb-5 w-fit ${
                  plan.highlight ? "bg-white/20 text-white" : "bg-white/10 text-white/70"
                }`}>
                ...
                <ul className="space-y-2.5 mt-auto">
                  {plan.features.map((f) => (
                    <li key={f} className={`flex items-start gap-2 text-xs ${plan.highlight ? "text-white" : "text-white/60"}`}>
                      <CheckCircle2 className={`w-3.5 h-3.5 shrink-0 mt-0.5 ${plan.highlight ? "text-white/80" : "text-white/40"}`} aria-hidden="true" />
                      {f}
                    </li>
                  ))}
                </ul>
      ...
      <section className="py-16 sm:py-20 bg-gray-50 border-t" aria-labelledby="faq-heading">
        <div className="max-w-2xl mx-auto px-4 sm:px-6">
          ...
          <div className="space-y-6">
            {[
              { q: "What counts as a document?", a: "Each file you upload counts as one document — regardless of page count. A 200-page PDF and a 5-page syllabus each count as one." },
              { q: "Do video transcriptions count toward my document limit?", a: "Yes. Each video or audio file you transcribe counts as one document against your monthly limit." },
