...
function RelatedCard({ article }: { article: KbArticle }) {
  return (
    <Link href={`/kb/articles/${article.id}`}>
      <a className="flex items-start gap-2 p-3 rounded-lg bg-[#1f2937] hover:bg-[#1f2937]/80 border border-gray-700/50 hover:border-[#0d9488]/40 transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#0d9488]">
        ...
        <div>
          <p className="text-sm text-white font-medium leading-snug">{article.title}</p>
  ...
  return (
    <div className="min-h-screen bg-[#111827] text-white">
      ...
      <header className="bg-[#1f2937] border-b border-gray-700/50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-4 flex items-center gap-2 text-sm flex-wrap">
          <Link href="/"><a className="text-[#0d9488] hover:underline focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#0d9488]">Remedy508</a></Link>
          <span className="text-gray-600" aria-hidden="true">/</span>
          <Link href="/kb"><a className="text-[#0d9488] hover:underline focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#0d9488]">Knowledge Base</a></Link>
          <span className="text-gray-600" aria-hidden="true">/</span>
          <span className="text-gray-400 truncate max-w-[200px]" aria-current="page">{article.section_name}</span>
        </div>
      </header>
      ...
      <main id="main-content" className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-white leading-tight">{article.title}</h1>
        ...
        {article.video_status === "published" && article.video_url ? (
          <div className="mb-8">
            <div className="rounded-xl overflow-hidden bg-black aspect-video border border-gray-700/50">
              <video
                ref={videoRef}
                controls
                preload="metadata"
          ...
          <div
            className="mb-8 rounded-xl border-2 border-dashed border-gray-700 bg-[#1f2937]/50 aspect-video flex flex-col items-center justify-center gap-3"
            role="img"
        ...
        {article.transcript ? (
          <section aria-labelledby="transcript-heading" className="mb-8">
            <h2 id="transcript-heading" className="text-lg font-semibold text-white mb-3">Transcript</h2>
            <div className="bg-[#1f2937] rounded-xl border border-gray-700/50 p-4 sm:p-6">
          ...
          <section aria-labelledby="transcript-heading" className="mb-8">
            <h2 id="transcript-heading" className="text-lg font-semibold text-white mb-3">Transcript</h2>
            <div className="bg-[#1f2937] rounded-xl border border-gray-700/50 p-4 sm:p-6">
              <p className="text-gray-400 text-sm italic">Transcript will be added when the video is published.</p>
            </div>
          </section>
        )}
        ...
        {related.length > 0 && (
          <section aria-labelledby="related-heading" className="mb-8">
            <h2 id="related-heading" className="text-lg font-semibold text-white mb-3">Related videos</h2>
        ...
        {/* Help footer */}
        <div className="bg-[#1f2937] rounded-xl border border-gray-700/50 p-4 sm:p-5 text-center">
